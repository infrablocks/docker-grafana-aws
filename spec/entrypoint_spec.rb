...
describe 'entrypoint' do
  ...
  describe 'by default' do
    ...
    it 'points at the correct installation directory' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{--homepath=/opt/grafana}))
    end

    it 'points at the correct configuration file' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{--config=/opt/grafana/conf/grafana.ini}))
    end
    ...
    it 'uses a data path of /var/opt/grafana' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{cfg:default.paths.data=/var/opt/grafana}))
    end

    it 'uses a logs path of /var/log/grafana' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{cfg:default.paths.logs=/var/log/grafana}))
    end

    it 'uses a plugins path of /opt/grafana/plugins' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{cfg:default.paths.plugins=/opt/grafana/plugins}))
    end

    it 'uses a provisioning path of /opt/grafana/provisioning' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(
              %r{cfg:default.paths.provisioning=/opt/grafana/provisioning}
            ))
    end

    it 'passes tracing argument to grafana-server command' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(/--tracing/))
    end

    it 'passes tracing-file argument to grafana-server command' do
      expect(process('/opt/grafana/bin/grafana-server').args)
        .to(match(%r{--tracing-file=/opt/grafana/trace.out}))
    end
    ...
    it 'renames GRAFANA_ environment variables to GF_' do
      ...
      expect(environment)
        .to(contain('GF_LOG_LEVEL=debug')
              .and(contain('GF_SERVER_HTTP_PORT=2999')))
    end
  ...
  describe 'when the plugins directory does not exist' do
    ...
    it 'uses grafana as the owning user of the plugins directory' do
      expect(file('/opt/grafana/storage/plugins'))
        .to(be_owned_by('grafana'))
    end

    it 'uses grafana as the owning group of the plugins directory' do
      expect(file('/opt/grafana/storage/plugins'))
  ...
  describe 'when aws profiles are provided' do
    ...
    it 'allows only the owning user to read the credentials file' do
      expect(file('/opt/grafana/.aws/credentials'))
        .to(be_mode(600))
    end

    it 'uses grafana as the owning user of the credentials file' do
      expect(file('/opt/grafana/.aws/credentials'))
        .to(be_owned_by('grafana'))
    end

    it 'uses grafana as the owning group of the credentials file' do
      expect(file('/opt/grafana/.aws/credentials'))
        .to(be_grouped_into('grafana'))
    end
  end
